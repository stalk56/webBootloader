...
(function() {
	...
	function formatFWSummary(device) {
		let isCRC = 0;
		let FWVer = 0;
		if((device.FWCRC != NaN) && (device.FWCRC != null)) {
			isCRC = device.FWCRC.value;
		}
		else {
			isCRC = false;
		}
		if((device.FWVersion != NaN) && (device.FWVersion != null)) {
			FWVer = device.FWVersion.value;
		}
		else {
			FWVer = 0;
		}
		let info = `FW version: ${FWVer}\nFW CRC matches: ${isCRC}`;
		return info;
	}
    ...
    document.addEventListener('DOMContentLoaded', event => {
        ...
        async function connect(device) {
			...
			dfuDisplay.textContent = formatFWSummary(device);
                ...
                async dfu_devices => {
                    ...
                    if (matching_devices.length == 0) {
                        ...
                        if (matching_devices.length == 1 || isNumWorks) { // For NumWorks, we want interface 0 ("Internal Flash")
                            ...
                            device = matching_devices[0].device_;
                            ...
                            device = await connect(new dfu.Device(device, interfaces[0]));
...
//                    }
					for(var i = 0; i < vid_pid.length; i++) {
						filters.push({'productId': vid_pid[i][1], 'vendorId': vid_pid[i][0]});
					}
                }
