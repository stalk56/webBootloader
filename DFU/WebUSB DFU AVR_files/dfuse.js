...
(function() {
    ...
    dfuse.Device = function(device, settings) {
        ...
        if (settings.name) {
			...
			this.FWCRC = dfuse.parseFWStatusDescriptor(settings.name);
			this.FWVersion = dfuse.parseFWVersionDescriptor(settings.name);
        }
    }
    ...
    dfuse.parseMemoryDescriptor = function(desc) {
        ...
        let segmentString = desc.substring(nameEndIndex);
		
		let memDescEndIndex = segmentString.indexOf("/FWSta");
		if (memDescEndIndex != -1) {
			segmentString = segmentString.substring(0,memDescEndIndex);
		}
		
		memDescEndIndex = segmentString.indexOf("/FWVer");
		if (memDescEndIndex != -1) {
			segmentString = segmentString.substring(0,memDescEndIndex);
		}
	...
	dfuse.parseFWStatusDescriptor = function(desc) {
		const nameIndex = desc.indexOf("/FWSta");
		if (nameIndex == -1) {
            return null
        }
		...
		let FWStaString = desc.substring(nameIndex+1);
		let start = FWStaString.indexOf("=")+1;
		let end = FWStaString.indexOf("/");
		if (start != -1) {
			if(end != -1) {
				FWStaString = FWStaString.substring(start, end);
			}
			else
			{
				FWStaString = FWStaString.substring(start);
			}
			if((parseInt(FWStaString)&(0x00000001)) == 1) //bit CRC
				crc = false;
			else
				crc = true;
		}
		...
		return {"name": "FWCRC", "value": crc};
	};
	
	dfuse.parseFWVersionDescriptor = function(desc) {
		const nameIndex = desc.indexOf("/FWVer");
		if (nameIndex == -1) {
            return null
        }
		
		let fwver = 0x00000000;
		
		let FWVerString = desc.substring(nameIndex+1);
		let start = FWVerString.indexOf("=")+1;
		let end = FWVerString.indexOf("/");
		if (start != -1) {
			if(end != -1) {
				FWVerString = FWVerString.substring(start, end);
			}
			else
			{
				FWVerString = FWVerString.substring(start);
			}
			fwver = parseInt(FWVerString);
		}
		else
		{
			fwver = 0;
		}
		return {"name": "FWVer", "value": fwver};
	};
